...
var CAG = {
    run: function (spawn) {
        ...
        ,MOVE,MOVE], newName, {
            memory: {
                role: 'transport_i',home:spawn,done:true,trade:false,link:-10,goal:-10,lab:false,trans:true,factory:false,func:true
            }
        });//300
        }
            ...
            MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], newName, {
                memory: {
                    role: 'transport',home:spawn,lab:false,trans:true,done:true,goal:-10,trade:false,factory:false
                }
            });//900
         //自动生成搬运工人
    }
    ...
    if (watcher.length<watchernum){
        ...
        if (spawn_p[0]!=undefined&&spawn_p[0].store.getUsedCapacity(RESOURCE_ENERGY)>0&&spawn_p[0].store.getUsedCapacity(RESOURCE_POWER)>0)
