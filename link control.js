...
var LK ={
    run:function()
    {
        ...
        for (var name in lc)
        {
            ...
            for (var name_ in li)
            {
            if (lc[name].store.getFreeCapacity()==0&&lc[name].room.name==li[name_].room.name&&li[name_].store.getFreeCapacity()>0)
            {
            ...
            for (var name_ in lo)
            {
            if (lc[name].store.getFreeCapacity()==0&&lc[name].room.name==lo[name_].room.name&&lo[name_].store.getFreeCapacity()>0)
            {
